...
percent <- psm.df %>%
   # Select sorghum proteins identified by "Sobic"
   filter(grepl("Sobic", Protein)) %>%
   mutate(Protein = sub("^(([^\\.]+\\.[^\\.]+)).*", "\\1", Protein))%>% #remove splice variant, just consider gene 
   # Group by Dataset and Protein
   group_by(Dataset, Protein) %>%
...
psm <- percent %>%
   ...
   ggplot(aes(x = reorder(Protein, -percentage), y = percentage, fill = Elution)) +
...
psm
   ...
   ggsave(plot = psm, filename= "C:/BRAVE/figures/percentpsm.png", 
          scale=2, 
          width = 12,
...
# Print results
cat("Common Proteins:\n", common_proteins, "\n")
cat("Proteins Unique to bt750:\n", unique_to_bt, "\n")
cat("Proteins Unique to sc750:\n", unique_to_sc, "\n")
...
venn.plot <- venn.diagram(
   x = list(
      bt750 = proteins_bt,       # Proteins from "bt750"
      sc750 = proteins_sc        # Proteins from "sc750"
   ),
...
#GO analysis of proteins in sc and bt 
#######################################################################
