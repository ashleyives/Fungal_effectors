...
database_file <- "C:/FASTA/HomosapiensPlusAAP9.fasta" #has contams/gluc
mods_file <- "C:/MSGFPlus_v20240326/MSGFPlus_Mods4_ANI.txt" 

# Get all .mzML files
mzML_files <- unique(list.files(path = "C:/BRAVE/msgf_PEPC", pattern = "\\.mzML$", full.names = TRUE, recursive = TRUE))

for (x in mzML_files) {
   print(x)
   
   input_file <- paste0("C:/BRAVE/msgf_PEPC", basename(x))
   output_file <- paste0("C:/BRAVE/msgf_PEPC/output", sub("\\.mzML$", "", basename(x)), ".mzid")
...
mzidlist <- unique(list.files(path = "C:/BRAVE/msgf_PEPC/output", pattern = "\\.mzid$", full.names = TRUE, recursive = TRUE))

for(x in unique(mzidlist)){
   print(x)
   
   exe_path <- "C:\\Users\\ives435\\Documents\\net48\\MzidToTsvConverter.exe"
   mzid_path <- paste0("C:\\BRAVE\\msgf_PEPC\\output\\", basename(x))
...
tsvlist <- unique(list.files(path = "C:/Nakai/fullfasta", pattern = "\\.tsv$", full.names = TRUE, recursive = TRUE))
